...
def _first_available_column(
    ...
    """Return the first present schema alias without requiring optional fields."""
    ...
    if cases is not None:
        ...
        if _available(work, "farm", "event_start", "event_end"):
            ...
            if not valid.empty:
                ...
                if sns is not None:
                    class_hue = "case_class" if "case_class" in valid else None
                    sns.boxplot(
                        data=valid,
                        x="farm",
                        y="event_duration_days",
                        hue=class_hue,
                        palette=(
                            {"Normal": "#4C78A8", "Anomaly": "#D55E00"}
                            if class_hue is not None
                            else None
                        ),
                        ax=ax,
                    )
...
if cases is not None and _available(cases, "farm", "is_anomaly"):
    tmp = cases.copy()
    tmp["farm"] = tmp["farm"].astype(str).str.strip()
    tmp["is_anomaly"] = _bool_series(tmp["is_anomaly"])
    dataset_table = (
        tmp.groupby("farm", as_index=False, sort=False, observed=False)
        .agg(
            cases=("is_anomaly", "size"),
            anomaly_cases=("is_anomaly", "sum"),
        )
    )
    dataset_sources = ["care_case_registry"]

    # Cell 3 names the case-level asset field ``asset_id``. Older persisted
    # registries used ``dataset_asset_id`` or ``metadata_asset_id``; later
    # audit/split tables use ``canonical_asset_id``. Asset counts are useful
    # descriptors but are not required for the CARE result, so resolve the
    # available schema instead of failing the complete publication export.
    asset_aliases = (
        "canonical_asset_id",
        "dataset_asset_id",
        "asset_id",
    ...
    asset_column = _first_available_column(tmp, asset_aliases)
    asset_source_name = "care_case_registry"
    asset_source = tmp
    if asset_column is None:
        for candidate_name in (
            "canonical_asset_summary",
            "care_asset_split_assignment",
            "care_case_split_registry",
        ):
            candidate_frame = table(candidate_name)
            candidate_column = _first_available_column(
                candidate_frame,
                asset_aliases,
            )
            if _available(candidate_frame, "farm") and candidate_column is not None:
                asset_source_name = candidate_name
                asset_source = candidate_frame
                asset_column = candidate_column
                break
    if asset_column is not None:
        asset_counts = (
            asset_source.loc[:, ["farm", asset_column]]
            .assign(farm=lambda frame: frame["farm"].astype(str).str.strip())
            .groupby("farm", observed=False)[asset_column]
            .nunique(dropna=True)
            .rename("assets")
        )
        ...
        if asset_source_name not in dataset_sources:
            dataset_sources.append(asset_source_name)
    else:
        skip_plot(
            "dataset_summary.assets",
            "No recognized asset identifier was available; the optional asset-count column was omitted.",
        )

    # Primary-signal dimensionality belongs to the farm schema in the current
    # pipeline, not necessarily to every case-registry row. Prefer a registry
    # column when present, otherwise merge the farm-level schema descriptor.
    signal_aliases = (
        "number_of_primary_signals",
        "primary_avg_signals",
        "primary_signals",
        "primary_signal_count",
    )
    signal_source_name = "care_case_registry"
    signal_source = tmp
    signal_column = _first_available_column(signal_source, signal_aliases)
    if signal_column is None:
        schema_frame = table("care_farm_schema_summary")
        schema_column = _first_available_column(schema_frame, signal_aliases)
        if _available(schema_frame, "farm") and schema_column is not None:
            signal_source_name = "care_farm_schema_summary"
            signal_source = schema_frame
            signal_column = schema_column
    if signal_column is not None:
        signal_counts = (
            signal_source.loc[:, ["farm", signal_column]]
            .assign(
                farm=lambda frame: frame["farm"].astype(str).str.strip(),
                **{signal_column: lambda frame: _numeric(frame[signal_column])},
            )
            .groupby("farm", observed=False)[signal_column]
            .max()
            .rename("primary_signals")
        )
        dataset_table = dataset_table.merge(
            signal_counts,
            left_on="farm",
        ...
        if signal_source_name not in dataset_sources:
            dataset_sources.append(signal_source_name)
    else:
        skip_plot(
            "dataset_summary.primary_signals",
            "No recognized primary-signal count was available; the optional signal-count column was omitted.",
        )

    dataset_table["normal_cases"] = dataset_table["cases"] - dataset_table["anomaly_cases"]
    farm_rank = {farm: rank for rank, farm in enumerate(FARM_ORDER)}
    dataset_table["_farm_rank"] = dataset_table["farm"].map(farm_rank).fillna(len(farm_rank))
    dataset_table = (
        dataset_table.sort_values(["_farm_rank", "farm"])
        .drop(columns="_farm_rank")
    ...
    export_paper_table(
        ...
        "dataset_summary",
        "Dataset summary",
        "Case and class counts for each wind farm, with available asset and primary-signal descriptors.",
        dataset_sources,
        "Main text",
        [
            "farm",
