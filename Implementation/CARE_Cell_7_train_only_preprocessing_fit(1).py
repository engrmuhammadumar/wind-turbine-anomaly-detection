...
CELL_7_POLICY = {
    ...
    "fit_row_rule": (
        ...
        "train_test value belongs to CARE's source-training partition, its "
        "status_type_id is CARE-normal (0 or 2), its timestamp is valid, at "
        "least one primary signal is finite, and the row is not "
        "synchronously all-zero across all finite primary signals."
    ),
    "source_partition_policy": (
        "Fit only source-training rows from model-split training cases. "
        "Source prediction rows are audited but never influence fitted "
        "statistics."
    ),
...
def cell_7_source_partition_label(value: Any) -> str:
    """Canonicalize CARE's per-row train_test partition label."""

    raw_label = cell_7_value_label(value)
    ...
    if normalized_label in SOURCE_TRAIN_PARTITION_LABELS:
        return "train"

    if normalized_label in SOURCE_PREDICTION_PARTITION_LABELS:
        return "prediction"

    return f"unrecognized::{normalized_label or '<EMPTY>'}"
...
train_source_partition_records: list[dict[str, Any]] = []
...
for farm_index, farm_name in enumerate(training_farms):
    ...
    farm_source_training_rows = 0
    farm_source_prediction_rows = 0
    farm_source_unknown_partition_rows = 0
    farm_status_accumulators: dict[str, dict[str, int]] = defaultdict(
        lambda: {
            "row_count": 0,
            "source_training_rows": 0,
            "source_prediction_rows": 0,
            "source_unknown_partition_rows": 0,
    ...
    farm_partition_accumulators: dict[str, dict[str, int]] = defaultdict(
        lambda: {
            "row_count": 0,
            "care_normal_status_rows": 0,
            "status_excluded_rows": 0,
            "valid_timestamp_rows": 0,
            "any_finite_rows": 0,
            "synchronous_all_zero_rows": 0,
            "fit_eligible_rows": 0,
    ...
    for farm_case_number, registry_row in enumerate(
        ...
        case_source_training_rows = 0
        case_source_prediction_rows = 0
        case_source_unknown_partition_rows = 0
        case_source_training_normal_status_rows = 0
        case_source_training_status_excluded_rows = 0
        case_fit_rows = 0
        ...
        for chunk in iter_care_csv_chunks(
            ...
            source_unknown_partition_rows = int(
                source_unknown_partition_mask.sum()
            )
            source_training_normal_status_rows = int(
                (source_training_mask & normal_status_mask).sum()
            )
            source_training_status_excluded_rows = int(
                (source_training_mask & ~normal_status_mask).sum()
            )
            ...
            case_source_training_rows += source_training_rows
            case_source_prediction_rows += source_prediction_rows
            case_source_unknown_partition_rows += (
                source_unknown_partition_rows
            )
            case_source_training_normal_status_rows += (
                source_training_normal_status_rows
            )
            case_source_training_status_excluded_rows += (
                source_training_status_excluded_rows
            )
            ...
            farm_source_training_rows += source_training_rows
            farm_source_prediction_rows += source_prediction_rows
            farm_source_unknown_partition_rows += (
                source_unknown_partition_rows
            )
            ...
            for status_label in pd.unique(status_labels):
                ...
                status_accumulator["source_training_rows"] += int(
                    (status_mask & source_training_mask).sum()
                )
                status_accumulator["source_prediction_rows"] += int(
                    (status_mask & source_prediction_mask).sum()
                )
                status_accumulator[
                    "source_unknown_partition_rows"
                ] += int(
                    (status_mask & source_unknown_partition_mask).sum()
                )
            ...
            for partition_label in pd.unique(partition_labels):
                partition_mask = partition_labels == partition_label
                partition_accumulator = farm_partition_accumulators[
                    str(partition_label)
                ]
                partition_accumulator["row_count"] += int(
                    partition_mask.sum()
                )
                partition_accumulator[
                    "care_normal_status_rows"
                ] += int((partition_mask & normal_status_mask).sum())
                partition_accumulator["status_excluded_rows"] += int(
                    (partition_mask & ~normal_status_mask).sum()
                )
                partition_accumulator["valid_timestamp_rows"] += int(
                    (partition_mask & valid_timestamp_mask).sum()
                )
                partition_accumulator["any_finite_rows"] += int(
                    (partition_mask & any_finite_mask).sum()
                )
                partition_accumulator[
                    "synchronous_all_zero_rows"
                ] += int(
                    (partition_mask & synchronous_all_zero_mask).sum()
                )
                partition_accumulator["fit_eligible_rows"] += int(
                    (partition_mask & fit_eligible_mask).sum()
                )
        ...
        train_preprocessing_fit_case_records.append(
            {
                ...
                "source_training_rows": case_source_training_rows,
                "source_prediction_rows": case_source_prediction_rows,
                "source_unknown_partition_rows": (
                    case_source_unknown_partition_rows
                ),
                "source_training_normal_status_rows": (
        ...
        train_preprocessing_signal_parameter_records.append(
            {
                ...
                "training_case_file_row_count": farm_total_rows,
                "source_training_partition_row_count": (
                    farm_source_training_rows
                ),
        ...
        train_status_operational_records.append(
            {
                ...
                "source_training_rows": int(
                    status_accumulator["source_training_rows"]
                ),
                "source_prediction_rows": int(
                    status_accumulator["source_prediction_rows"]
                ),
                "source_unknown_partition_rows": int(
                    status_accumulator[
                        "source_unknown_partition_rows"
    ...
    for partition_label, partition_accumulator in sorted(
        farm_partition_accumulators.items(),
        key=lambda item: item[0],
    ):
        partition_row_count = int(partition_accumulator["row_count"])
        train_source_partition_records.append(
            {
                "farm": farm_name,
                "source_partition": partition_label,
                "source_training_partition": bool(
                    partition_label == "train"
                ),
                "row_count": partition_row_count,
                "row_fraction_within_farm": (
                    partition_row_count / farm_total_rows
                    if farm_total_rows > 0
                    else np.nan
                ),
                "care_normal_status_rows": int(
                    partition_accumulator["care_normal_status_rows"]
                ),
                "status_excluded_rows": int(
                    partition_accumulator["status_excluded_rows"]
                ),
                "valid_timestamp_rows": int(
                    partition_accumulator["valid_timestamp_rows"]
                ),
                "any_finite_rows": int(
                    partition_accumulator["any_finite_rows"]
                ),
                "synchronous_all_zero_rows": int(
                    partition_accumulator[
                        "synchronous_all_zero_rows"
                ...
                "fit_eligible_rows": int(
                    partition_accumulator["fit_eligible_rows"]
                ),
                "fit_eligible_fraction": (
                    partition_accumulator["fit_eligible_rows"]
                    / partition_row_count
...
train_source_partition_audit = pd.DataFrame(
    train_source_partition_records
)
...
if not train_source_partition_audit.empty:
    train_source_partition_audit = (
        train_source_partition_audit
        .sort_values(["farm", "source_partition"], kind="stable")
        .reset_index(drop=True)
    )
...
train_preprocessing_farm_summary = (
    ...
    .agg(
        ...
        source_training_rows=("source_training_rows", "sum"),
        source_prediction_rows=("source_prediction_rows", "sum"),
        source_unknown_partition_rows=(
            "source_unknown_partition_rows",
            "sum",
        ),
...
train_preprocessing_farm_summary["fit_eligible_fraction"] = (
    ...
    / train_preprocessing_farm_summary["source_training_rows"]
)
train_preprocessing_farm_summary[
    "fit_eligible_fraction_of_all_case_rows"
] = (
    train_preprocessing_farm_summary["fit_eligible_rows"]
...
partition_row_totals = (
    train_source_partition_audit
    .groupby("farm", as_index=False, sort=False)["row_count"]
    .sum()
    .rename(columns={"row_count": "partition_row_count"})
)
partition_conservation = train_preprocessing_farm_summary[
    ["farm", "training_source_rows"]
].merge(
    partition_row_totals,
    on="farm",
...
if not partition_conservation["training_source_rows"].eq(
    partition_conservation["partition_row_count"]
).all():
    cell_7_validation_errors.append(
        "train_test partition counts do not conserve all training-case rows."
    )

unknown_source_partition_rows = int(
    train_preprocessing_farm_summary[
        "source_unknown_partition_rows"
    ].sum()
)

if unknown_source_partition_rows > 0:
    cell_7_validation_errors.append(
        f"{unknown_source_partition_rows} training-case rows have missing "
        "or unrecognized train_test partition values."
    )

nontraining_partition_fit_rows = int(
    train_source_partition_audit.loc[
        ~train_source_partition_audit[
            "source_training_partition"
        ].eq(True),
        "fit_eligible_rows",
    ].sum()
)

if nontraining_partition_fit_rows > 0:
    cell_7_validation_errors.append(
        f"{nontraining_partition_fit_rows} source-prediction or unknown "
        "partition rows incorrectly contributed to preprocessing fit."
    )
...
case_partition_conservation_mask = (
    train_preprocessing_fit_case_audit["source_training_rows"]
    + train_preprocessing_fit_case_audit["source_prediction_rows"]
    + train_preprocessing_fit_case_audit[
        "source_unknown_partition_rows"
    ]
    != train_preprocessing_fit_case_audit["row_count"]
...
if case_partition_conservation_mask.any():
    cell_7_validation_errors.append(
        f"{int(case_partition_conservation_mask.sum())} training cases fail "
        "source-partition row conservation."
    )

case_training_status_conservation_mask = (
    train_preprocessing_fit_case_audit[
        "source_training_normal_status_rows"
    ]
    + train_preprocessing_fit_case_audit[
...
if case_training_status_conservation_mask.any():
    cell_7_validation_errors.append(
        f"{int(case_training_status_conservation_mask.sum())} training cases "
        "fail source-training normal/abnormal-status conservation."
    )
...
source_training_status_excluded_row_count = int(
    train_preprocessing_fit_case_audit[
        "source_training_status_excluded_rows"
    ].sum()
)
source_prediction_excluded_row_count = int(
    train_preprocessing_fit_case_audit["source_prediction_rows"].sum()
)
...
if source_training_status_excluded_row_count > 0:
    cell_7_validation_warnings.append(
        f"{source_training_status_excluded_row_count} source-training rows "
        "have CARE-abnormal or missing status IDs and were excluded from "
        "normal-behavior preprocessing fit."
    )

if source_prediction_excluded_row_count > 0:
    cell_7_validation_warnings.append(
        f"{source_prediction_excluded_row_count} source-prediction rows "
        "were audited but excluded from preprocessing fit."
    )
...
cell_7_constraint_records = [
    ...
    {
        "constraint": "only_source_training_partition_rows_fit",
        "passed": nontraining_partition_fit_rows == 0,
        "observed": nontraining_partition_fit_rows,
        "expected": 0,
    },
    {
        "constraint": "source_partition_values_recognized",
        "passed": unknown_source_partition_rows == 0,
        "observed": unknown_source_partition_rows,
        "expected": 0,
    },
    {
    ...
    {
        "constraint": "source_partition_row_conservation",
        "passed": bool(
            partition_conservation["training_source_rows"].eq(
                partition_conservation["partition_row_count"]
            ).all()
        ),
        "observed": int(
            partition_conservation["training_source_rows"].sub(
                partition_conservation["partition_row_count"]
            ).abs().sum()
        ),
        "expected": 0,
...
train_source_partition_audit.to_csv(
    TABLE_DIR / "care_train_source_partition_audit.csv",
    index=False,
)
...
cell_7_manifest = {
    ...
    "source_training_partition_labels": sorted(
        SOURCE_TRAIN_PARTITION_LABELS
    ),
    "source_prediction_partition_labels": sorted(
        SOURCE_PREDICTION_PARTITION_LABELS
    ),
    ...
    "unknown_source_partition_row_count": (
        unknown_source_partition_rows
    ),
...
print("\n" + "=" * 80)
print("TRAIN SOURCE-PARTITION AUDIT")
print("=" * 80)
display(train_source_partition_audit)
...
print(
    "Source-train status exclusions: "
    f"{source_training_status_excluded_row_count}"
)
print(
    "Source-prediction exclusions  : "
    f"{source_prediction_excluded_row_count}"
)
...
print("  - train_source_partition_audit")
...
print("  - care_train_source_partition_audit.csv")
