...
=============================================================================
MSSP UPGRADE
Cell 15B: resolution-amended frozen UORED-VAFCLS feature extraction
=============================================================================

Run after Cell 15A in the same notebook session. This cell uses only the
locked Cell 14 loader specification, the locked resolution amendment and the
unchanged v4 feature extractor.

Registered analysis unit
------------------------
Each of the 60 bearing states is divided into all complete, consecutive,
non-overlapping 54-revolution segments. At 42 kHz and the registered 1750 RPM
fallback, this gives 5 segments per state and 300 segments in total. This is
the smallest exact FFT-friendly amendment that retains both BPFI and BPFO
under the unchanged v4 collision rule while preserving five segments/state.
...
if missing_cell15_names:
    raise RuntimeError(
        "Run Cells 13, 14 and 15A after the v4 extractor cells. Missing: "
...
if str(V5_RESOLUTION_AMENDMENT_HASH) != EXPECTED_AMENDMENT_HASH:
    raise RuntimeError("The locked Cell 15A amendment hash has changed")

amendment_path = Path(V5_RESOLUTION_AMENDMENT_PATH)
if not amendment_path.exists():
    raise FileNotFoundError(
        f"Cell 15A resolution amendment is missing: {amendment_path}"
    )
with amendment_path.open("r", encoding="utf-8") as handle:
    stored_amendment_record = json.load(handle)
if stored_amendment_record.get("amendment_sha256") != (
    EXPECTED_AMENDMENT_HASH
):
    raise RuntimeError("Stored Cell 15A amendment has changed")
if stored_amendment_record["amendment"].get(
    "parent_registration_sha256"
) != EXPECTED_CELL13_HASH:
    raise RuntimeError("Cell 15A amendment has the wrong parent registration")
...
if canonical_digest(stored_amendment_record["amendment"]) != (
    ...
    raise RuntimeError("Stored Cell 15A amendment failed its hash check")


CELL15_CONFIG = {
    "method_version": "v5.0-uored-resolution-amended-extraction",
    "source_registration_sha256": EXPECTED_CELL13_HASH,
    "source_resolution_amendment_sha256": EXPECTED_AMENDMENT_HASH,
...
if int(segment_counts.min()) < int(
    CELL15_CONFIG["minimum_complete_segments_per_state"]
):
    raise RuntimeError("A state has fewer than five complete segments")

# Outcome-blind preflight: fail before reading any raw signal if the locked
# resolution amendment does not retain both primary physical orders.
resolution_rows = []
for values in UORED_SEGMENT_DESIGN[
    ["fs", "fr", "segment_samples"]
].drop_duplicates().itertuples(index=False):
    df_resolution = float(values.fs) / int(values.segment_samples)
    kept, dropped = usable_orders_v3(
        bearing_key=UORED_BEARING_KEY,
        fr=float(values.fr),
    ...
    if not {"BPFI", "BPFO"}.issubset(kept):
        raise RuntimeError(
            "Cell 15A resolution preflight failed: BPFI/BPFO are not both "
            f"usable. kept={sorted(kept)}, dropped={sorted(dropped)}"
        )
    if "BSF" in kept:
        raise RuntimeError(
            "Unexpected BSF retention conflicts with the locked amendment"
        )
    resolution_rows.append({
        ...
        "df_resolution": df_resolution,
        "kept_orders": ",".join(sorted(kept)),
        "dropped_orders": ",".join(sorted(dropped)),
    })

UORED_RESOLUTION_PREFLIGHT = pd.DataFrame(resolution_rows)
...
print("Resolution-amended frozen UORED v5 feature extraction")
print(f"  configuration SHA-256 : {CELL15_CONFIG_HASH}")
print(f"  amendment SHA-256     : {EXPECTED_AMENDMENT_HASH}")
...
print("\n" + "=" * 104)
print("CELL 15B RESOLUTION-AMENDED UORED FEATURE-EXTRACTION REPORT")
print("=" * 104)
print(f"Configuration SHA-256  : {CELL15_CONFIG_HASH}")
print(f"Amendment SHA-256      : {EXPECTED_AMENDMENT_HASH}")
print(f"Segment design SHA-256 : {UORED_SEGMENT_DESIGN_HASH}")
