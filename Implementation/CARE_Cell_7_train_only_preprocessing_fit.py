...
CELL_7_POLICY = {
    ...
    "fit_row_rule": (
        ...
        "timestamp is valid, at least one primary signal is finite, and the "
        "row is not synchronously all-zero across all finite primary signals."
    ),
    ...
    "missing_indicator_rule": (
        ...
        "value in the training source rows."
    ),
    "status_type_id_policy": (
        "Audit jointly with synchronous all-zero state; do not infer or drop "
        "a status category in this cell."
    ),
...
for farm_index, farm_name in enumerate(training_farms):
    ...
    for feature_index, signal_name in enumerate(primary_signal_columns):
        ...
        add_missing_indicator = bool(
            keep_for_model and raw_nonfinite_count > 0
        )
        ...
        train_preprocessing_signal_parameter_records.append(
            {
                ...
                "fit_nonfinite_count": int(
                    farm_fit_rows - finite_fit_count
                ),
        ...
        train_status_operational_records.append(
            {
                ...
                "status_used_as_exclusion_rule": False,
            }
        )
